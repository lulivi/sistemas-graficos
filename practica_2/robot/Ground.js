...
class Ground extends THREE.Object3D {
    ...
    intersectBoxes (b1, b2) {
        ...
        vectorBetweenBoxes.subVectors (new THREE.Vector2 (b1.position.x,
                                                          b1.position.z),
                                       new THREE.Vector2 (b2.position.x,
                                                          b2.position.z));
    ...
    /**
     * @param event - Mouse information
     * @param action - Which action is going to be processed: start
     adding or finish.
    */
    addBox (event, action) {
    ...
    /**
     * @param event - Mouse information
     * @param action - Which action is going to be processed: select a
     box, move it, rotate it or finish the action.
    */
    moveBox (event, action) {
        ...
        case TheScene.SELECT_BOX :
            var mouse = this.getMouse (event);
            this.raycaster.setFromCamera (mouse, scene.getCamera());
            var pickedObjects = this.raycaster.intersectObjects
            (this.boxes.children);
            if (pickedObjects.length > 0) {
    ...
    takeBox (position) {
        if (this.boxes.children.length === 0) // There are no boxes
            return null;
        var minDistance = position.distanceToSquared
        (this.boxes.children[0].position);
        var nearestBox = 0;
        var distance = 0;
        for (var i = 1; i < this.boxes.children.length; i++) {
            distance = position.distanceToSquared
            (this.boxes.children[i].position);
