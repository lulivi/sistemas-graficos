...
class Crane extends THREE.Object3D {
	...
	this.trolley      = null;
	this.string       = null;
	// The string length is 1 at the beginning. So, the current length
	// is the scale factor
	this.stringLength = 1;
	this.hook         = null;
	this.box          = null;
	...
	// A way of feedback, a red jail will be visible around the crane
	// when a box is taken by it
    ...
    computeStringLength () {
	// stringLenght = base height + crane height - trolley height -
	// hook height - height of the hook to the ground. So,
	// stringLength = baseHookHeight + craneHeight - trolleyHeight -
	// baseHookHeight - height;
	...
	if (this.distanceMin <= aDistance && aDistance <=
	    this.distanceMax) {
    ...
    */
    ...
    /**
     * @param world - Whether the returned position is referenced to the
     World Coordinates System (Crane.WORLD) or is referenced to the crane
     position (Crane.LOCAL)
     ...
     * @return The new height of the hook, on the top of the box. Zero
     if no box is taken
    */
    takeBox (aBox) { 
	if (this.box === null) {
	    ...
	    this.box = aBox;
	    var newHeight = this.box.position.y +
		this.box.geometry.parameters.height;
	    this.heightMin = this.box.geometry.parameters.height;
	    this.box.position.x = 0;
	    this.box.position.y =
		-this.box.geometry.parameters.height-this.baseHookHeight;
